...
public class PostService {
    ...
    public Page<Post> getFeed(Long userId, int page, int size) {
        ...
        result.getContent().forEach(post -> {
            populateCounts(post);
            if (post.getAttachmentFileType() != null) {
                System.out.println("📤 Post #" + post.getId() + " - Attachment type: " + post.getAttachmentFileType());
            }
        });
        return result;
    }
    ...
    // --- Attachements ---
    @Transactional
    public void uploadPostAttachment(Long postId, Long userId, org.springframework.web.multipart.MultipartFile file) throws java.io.IOException {
        Post post = getOwnedPost(postId, userId);
        AttachmentData attachment = attachmentService.uploadAttachment(file);

        System.out.println("💾 Storing attachment - Type: " + attachment.getFileType() +
                           " | File: " + attachment.getFileName() +
                           " | Size: " + attachment.getFileSize());

        post.setAttachmentFileName(attachment.getFileName());
        post.setAttachmentFileUrl(attachment.getFileUrl());
        post.setAttachmentFileType(attachment.getFileType());
        post.setAttachmentFileSize(attachment.getFileSize());

        postRepository.save(post);
        System.out.println("✅ Post saved with attachment type: " + post.getAttachmentFileType());
    }

    @Transactional
    public void removePostAttachment(Long postId, Long userId) {
        Post post = getOwnedPost(postId, userId);
        if (post.getAttachmentFileUrl() != null) {
            attachmentService.deleteAttachment(post.getAttachmentFileUrl());
        }
        post.setAttachmentFileName(null);
        post.setAttachmentFileUrl(null);
        post.setAttachmentFileType(null);
        post.setAttachmentFileSize(null);
        postRepository.save(post);
    }

    @Transactional
    public void uploadCommentAttachment(Long commentId, Long userId, org.springframework.web.multipart.MultipartFile file) throws java.io.IOException {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new RuntimeException("Commentaire non trouvé"));
        ...
        comment.setAttachmentFileName(attachment.getFileName());
        comment.setAttachmentFileUrl(attachment.getFileUrl());
        comment.setAttachmentFileType(attachment.getFileType());
        comment.setAttachmentFileSize(attachment.getFileSize());

        commentRepository.save(comment);
    }

    @Transactional
    public void removeCommentAttachment(Long commentId, Long userId) {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new RuntimeException("Commentaire non trouvé"));
        ...
        if (comment.getAttachmentFileUrl() != null) {
            attachmentService.deleteAttachment(comment.getAttachmentFileUrl());
        }
        comment.setAttachmentFileName(null);
        comment.setAttachmentFileUrl(null);
        comment.setAttachmentFileType(null);
        comment.setAttachmentFileSize(null);
        commentRepository.save(comment);
    }

    @Transactional(readOnly = true)
    public byte[] downloadAttachment(String filename) throws java.io.IOException {
        return attachmentService.serveAttachment(filename);
    }
